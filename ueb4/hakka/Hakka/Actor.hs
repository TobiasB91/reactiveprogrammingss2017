...
data ActorSystem m = ActorSystem {  
  ...
  tellIO :: ActorRef m -> m -> IO () -- ^ For interacting with actor from outside an actor system
}

data ActorSystemState m = ActorSystemState {
  actors :: [(ActorPath,Inbox m)]
}
